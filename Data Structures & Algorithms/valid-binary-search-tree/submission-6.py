...
class Solution:
    ...
    # 4. left right - already has lower, need to apply upper
        ...
        if upper != 0 and not upper:
            if val <= lower:
                return False
            else:
                return self.check(node.left, lower, val) and self.check(node.right, val, None)
        elif lower != 0 and not lower:
            if val >= upper:
                return False
            else:
                return self.check(node.left, None, val) and self.check(node.right, val, upper)
        else:
